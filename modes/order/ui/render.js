...
import { ACTIVE_CAP, BENCH_CAP, CLANS, LEVELS, MAX_HEARTS, RANK_AT, SLOT_MILESTONES, activeSlots, clanCounts, modifierLabel, rankFor } from '../engine/config.js';
...
function phaseActionLabel(ctx) {
  ...
  if (ctx.run.phase === PHASE.FOCUS) return 'Strike'; // post-roll: re-order, then strike
  return 'Roll';
}
...
function renderActive(ctx) {
  ...
  // ORDER TEST (post): in FOCUS the dice show their ROLLED faces so the player
  // re-orders known values before striking. (Pre never lingers in FOCUS.)
  const focusRows = (run.phase === PHASE.FOCUS && run.lastPreview) ? run.lastPreview.diceRes : null;
  ...
  for (let i = 0; i < ACTIVE_CAP; i++) {
    ...
    if (run.phase === PHASE.ARRANGE || (run.phase === PHASE.FOCUS && run.orderTiming === 'post')) slot.classList.add('movable');
    if (dr) {
...
function renderStatus(ctx) {
  ...
  const recap = run.lastStrike && !run.lastStrike.cleared
    ...
    : '';
  $('readout').innerHTML = (run.phase === PHASE.FOCUS && run.orderTiming === 'post')
    ? 'drag to re-order · then <b>Strike</b>'
    : [recap, run.wager ? 'wager ×2' : ''].filter(Boolean).join(' · ');
  setTarget(target);
