...
const SAVE_KEY = 'opr-order-save-v1';
...
// ORDER TEST (post variant): the round is TWO taps. ROLL reveals the faces and
// stops in FOCUS; the player drags the rolled dice into order; STRIKE resolves.
async function doRoll() {
  ...
  busy = false;
  refresh(); // the rolled lane is now re-orderable; the verb reads STRIKE
}
...
function restart(difficulty = run.difficulty, orderTiming = run.orderTiming || 'pre') {
  run = createRun(Math.random, difficulty, orderTiming);
  runStarted = true;
  selected = null;
  runSeq++;
...
function onAction() {
  ...
  if (run.phase === PHASE.ARRANGE) (run.orderTiming === 'post' ? doRoll() : doFight());
  else if (run.phase === PHASE.FOCUS) doStrike();
...
const menu = initMenu({
  ...
  newRun: (diffId, orderTiming) => restart(diffId, orderTiming),
  runAgain: () => restart(),
});
refresh();
menu.openMenu();
