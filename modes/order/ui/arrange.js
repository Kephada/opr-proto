...
import { PHASE, combineDice, currentRewardOffers, currentShopOffers, moveActive, promoteBench, reorderRolled, swapActiveWithBench } from '../engine/run.js';
...
export function initArrange(io) {
  ...
  function applyDrop(dragZone, from, t) {
    const run = io.getRun();
    let next = run;
    // ORDER TEST (post): after the roll, the lane only RE-ORDERS - no combine,
    // no bench, faces are locked. Reorder keeps each die's rolled face.
    if (run.phase === PHASE.FOCUS && run.orderTiming === 'post') {
      if (dragZone === 'active' && t.zone === 'active') {
        next = reorderRolled(run, from, t.idx);
        io.setRun(next);
        io.setSelected(null);
        if (next !== run) tone(520, .05, 'triangle', .045);
        io.refresh();
      }
      return;
    }
  ...
  function markDropTarget(x, y, dragZone) {
    ...
    if (!slot) return;
    const run = io.getRun();
    // Post-roll lane: reorder only, so just the neutral drop highlight.
    if (run.phase === PHASE.FOCUS) { slot.classList.add('drop-target'); return; }
  ...
  function startDrag(e, zone, i) {
    if (io.isBusy()) return;
    const run = io.getRun();
    // Post-roll reorder: dragging the rolled lane is allowed in FOCUS too.
    const reorderOnly = run.phase === PHASE.FOCUS && run.orderTiming === 'post';
    if (run.phase !== PHASE.ARRANGE && !reorderOnly) {
      if (zone === 'active') tapSlot(zone, i);
      return;
    }
    if (reorderOnly && zone !== 'active') return;
    e.preventDefault();
    resumeAudio();
    startAmbient();
    dragFrom = i;
