...
export function initMenu(io) {
  ...
  // ---- Run setup: difficulty + ORDER TIMING (the A/B variable under test) --
  const SETUP_KEY = 'opr-order-setup-v1';
  const setup = { diff: 'normal', order: 'pre' };
  try { Object.assign(setup, JSON.parse(localStorage.getItem(SETUP_KEY)) || {}); } catch (e) { /* fresh */ }
  if (!['pre', 'post'].includes(setup.order)) setup.order = 'pre';

  const ORDERS = [
    { id: 'pre', name: 'Before roll', hint: 'Lock the order, THEN roll and watch — the real game.' },
    { id: 'post', name: 'After roll', hint: 'Roll first, re-order the revealed dice, then Strike.' },
  ];
  ...
  function renderSetup() {
    ...
    $('setupMode').innerHTML = ORDERS.map(o =>
      `<button class="opt-toggle${setup.order === o.id ? ' on' : ''}" type="button" data-sorder="${o.id}">${o.name}</button>`).join('');
    $('setupHint').textContent = (ORDERS.find(o => o.id === setup.order) || ORDERS[0]).hint;
    ...
    menuEl.querySelectorAll('[data-sorder]').forEach(b => b.addEventListener('click', () => {
      setup.order = b.dataset.sorder;
      saveSetup();
  ...
  $('mStart').addEventListener('click', () => {
    ...
    closeMenu();
    io.newRun(setup.diff, setup.order);
  });
