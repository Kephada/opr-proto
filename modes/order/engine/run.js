...
import { findGhostFix, movedRollState, previewTimeline, rollTimeline, strikeTimeline } from './resolve.js';
...
export function createRun(rng = Math.random, difficulty = 'normal', orderTiming = 'pre') {
  ...
  return {
    ...
    difficulty: diff.id,
    // ORDER TEST: 'pre' = lock order at the FIGHT tap (the real game); 'post' =
    // roll first, then re-order the revealed dice, then strike.
    orderTiming,
    levelIdx: 0,
    fightDmg: 0,
    fightVenom: 0,
...
// ORDER TEST (post variant): reorder the dice AFTER the roll. Moves the rolled
// die from->to, keeping its rolled face, and recomputes the preview so the
// player can re-sequence known faces before striking. FOCUS phase only.
export function reorderRolled(run, from, to) {
  if (run.phase !== PHASE.FOCUS || !run.lastRoll) return run;
  const active = run.build.active;
  if (from === to || !active[from] || to < 0 || to >= active.length) return run;
  const build = cloneBuild(run.build);
  ...
  build.active.splice(to, 0, moved);
  const foe = combatFoe(run);
  const lastRoll = movedRollState(run.lastRoll, foe, from, to);
  const lastPreview = previewTimeline(build, foe, lastRoll);
  return { ...run, build, lastRoll, lastPreview };
}
