...
// PATH 1 — ELEMENTS copy: every die PAINTS its element on the foe; a hit on
// a DIFFERENT paint can REACT (directional table, docs/proto/PATH-1-ELEMENTS.md).
// Ranks shrink to: r1 = clan dice +1, r2 = clan reactions +50%, r3 = the
// named specials (the love objects stay).
export const CLANS = [
  {
    id: CLAN.EMBER, name: 'Fire', color: 'red', icon: 'FI',
    desc: 'IGNITE (fire after poison): poison detonates ×2, stacks consumed.',
    ranks: [
      'Fire dice +1.',
      'Fire reactions +50%.',
      'CHAIN DETONATION: fire crits explode, and explosions chain.',
    ],
  },
  {
    id: CLAN.VENOM, name: 'Poison', color: 'green', icon: 'PO',
    desc: 'CORRODE (poison after stone): +half-face poison. Poison ignores Armor.',
    ranks: [
      'Poison dice +1.',
      'Poison reactions +50%.',
      'THE PLAGUE: poison dice apply 4 poison; poison damage doubles.',
    ],
  },
  {
    id: CLAN.STORM, name: 'Storm', color: 'blue', icon: 'ST',
    desc: 'OVERLOAD (storm after fire): +face bonus damage.',
    ranks: [
      'Storm dice +1.',
      'Storm reactions +50%.',
      'TEMPEST ECHO: charge next +3. Your last die strikes twice.',
    ],
  },
  {
    id: CLAN.STONE, name: 'Stone', color: 'grey', icon: 'SN',
    desc: 'SHATTER (stone after storm): breaks Armor/Ward, +2.',
    ranks: [
      'Stone dice +1.',
      'Stone reactions +50%.',
  ...
  },
  {
    ...
    desc: 'TRANSMUTE (gold after any paint): pays gold by tier.',
    ranks: [
      'Gold dice +1.',
      'Gold reactions +50%.',
...
// PATH 1 — ELEMENTS: species keep their NAME and SIGIL only (identity for
// the same-name combine chase). Face abilities are gone; a die's job is its
// ELEMENT — paint the foe, react off the previous paint.

// Keyword language (one vocabulary everywhere, UI bolds them):
// CRIT = the die shows its TOP FACE - and charge counts toward it, so a
// charged 4 on a d6+2 IS a crit. PAINT = the element the foe wears.
export const KEYWORD_LEGEND = 'CRIT = the die shows its top face. Charge counts toward it. A crit DOUBLES its reaction.';
...
export const SPECIES = [
  { id: 'cinder', clan: CLAN.EMBER, name: 'Ember' },
  { id: 'fusewick', clan: CLAN.EMBER, name: 'Fuse' },
  { id: 'fangling', clan: CLAN.VENOM, name: 'Fang' },
  { id: 'maw', clan: CLAN.VENOM, name: 'Eater' },
  { id: 'stormeye', clan: CLAN.STORM, name: 'Mirror' },
  { id: 'galecaller', clan: CLAN.STORM, name: 'Gust' },
  { id: 'cornerstone', clan: CLAN.STONE, name: 'Anchor' },
  { id: 'bulwark', clan: CLAN.STONE, name: 'Shield' },
  { id: 'gilded', clan: CLAN.COIN, name: 'Coin' },
  { id: 'tithe', clan: CLAN.COIN, name: 'Taxman' },
];
...
// The die tooltip ability line IS its element's reaction line (paint side).
export const REACTION_LINES = {
  [CLAN.EMBER]: 'Paints FIRE. Storm hit after = OVERLOAD.',
  [CLAN.VENOM]: 'Paints POISON. Fire hit after = IGNITE.',
  [CLAN.STORM]: 'Paints STORM. Stone hit after = SHATTER.',
  [CLAN.STONE]: 'Paints STONE. Poison hit after = CORRODE.',
  [CLAN.COIN]: 'Paints GOLD. Gold hit on any paint = TRANSMUTE.',
};
export const speciesAbilityText = d => REACTION_LINES[d.element] || '';
...
export const overkillDivisor = levelIdx => (levelIdx >= 6 ? 4 : 5);
export const COIN_KILL_BONUS = [0, 0, 0, 7]; // by coin rank (MIDAS only; r1/r2 are generic now)
