...
import {
  CLAN, CLANS, COIN_KILL_BONUS, DRAFT_REROLL_COST, LEVELS, MAX_HEARTS,
  SHOP_AFTER, SHOP_REROLL_COST, STARTER, activeSlots, canCombine, clanRanks,
  cloneBuild, combinedDie, difficultyInfo, draftOffers, killGold,
  overkillDivisor, pickTheme, shopOffers, slotsForFight, themedLevels,
} from './config.js';
...
export function currentFoe(run) {
  ...
  const foe = {
    ...level,
    baseTarget: level.target,
    // Per-fight resist rotates with the ladder (levelIdx % 5): that
    // element's reactions fizzle. Painting with it still works.
    immuneElement: CLANS[run.levelIdx % CLANS.length].id,
  };
  if (run.fightShattered) foe.modifier = null; // SHATTER holds for the fight
  const dmg = run.fightDmg || 0;
  if (dmg) foe.target = Math.max(1, level.target - dmg);
  return foe;
}
...
function combatFoe(run) {
  return {
    ...
    overkillDiv: overkillDivisor(run.levelIdx),
  };
}
...
export function strikeCurrentFight(run, rng = Math.random) {
  ...
  // The foe still stands: it strikes back and KEEPS its wounds - damage,
  // venom stacks, and a SHATTERED armor/ward all carry into the next roll.
  return {
    ...base,
    phase: PHASE.ARRANGE,
    fightDmg: (run.fightDmg || 0) + lastStrike.total,
    fightVenom: lastStrike.venomCarry || 0,
    fightShattered: run.fightShattered || !!lastStrike.shattered,
    lastRoll: null,
    lastPreview: null,
  };
}

// Draft reroll: 3g flat, once per draft.
export function rerollDraft(run, rng = Math.random) {
  if (run.phase !== PHASE.REWARD || run.draftRerolled) return run;
  if (run.gold < DRAFT_REROLL_COST) return run;
  const draftStock = draftOffers(run.levelIdx, run.build, rng, true);
  return { ...run, gold: run.gold - DRAFT_REROLL_COST, draftStock, draftRerolled: true };
}
