...
// Paint names as the player reads them (clan display names, not engine ids).
const PAINT_NAMES = { fire: 'FIRE', venom: 'POISON', storm: 'STORM', stone: 'STONE', coin: 'GOLD' };
...
const LOG_TEXT = {
  ...
  'clan-bonus': e => `+${e.amount} clan`,
  midas: e => `MIDAS +${e.amount}`,
  'coin-max': e => `pays ${e.amount} gold`,
  'tempest-echo': () => 'TEMPEST ECHO',
  reaction: e => e.name === 'TRANSMUTE' ? `TRANSMUTE! +${e.amount}g`
    : e.name === 'CORRODE' ? `CORRODE! +${e.amount} poison`
    : `${e.name}! +${e.amount}`,
  immune: e => `${e.name} fizzles - immune`,
  paint: e => `paints ${PAINT_NAMES[e.element] || e.element}`,
...
export async function animateThrow(refs, r) {
  ...
  hits.innerHTML = '';
  // The foe's per-fight resist stands through the cascade (rollView wiped
  // the icon row); the paint splat joins it live as dice land.
  if (refs.debuffs && r.immuneElement) {
    refs.debuffs.innerHTML = debuffIcon('immune', 0,
      `Immune: ${PAINT_NAMES[r.immuneElement] || r.immuneElement} reactions do nothing`,
      `el-${r.immuneElement}`);
  }
  tray.innerHTML = '';
  ...
  for (const tick of r.ticks) {
    ...
    const reaction = tick.events.find(e => e.type === 'reaction');
    const immuneEv = tick.events.find(e => e.type === 'immune');
    const applied = tick.events.filter(e => e.type === 'venom-apply').reduce((s, e) => s + e.amount, 0)
      + (reaction?.name === 'CORRODE' ? reaction.amount : 0);
    ...
    // IGNITE consumed every stack: the poison icon burns away.
    if (reaction?.name === 'IGNITE' && refs.debuffs) {
      venomStacks = 0;
      refs.debuffs.querySelector('.debuff.poison')?.remove();
    }
    // The foe wears its paint: one splat icon, recolored as each die lands.
    const paintEv = tick.events.find(e => e.type === 'paint');
    if (paintEv && refs.debuffs) {
      refs.debuffs.querySelector('.debuff.paint')?.remove();
      refs.debuffs.insertAdjacentHTML('beforeend', debuffIcon('paint', 0,
        `Painted ${PAINT_NAMES[paintEv.element] || paintEv.element}`, `el-${paintEv.element}`));
      pulseDebuff(refs.debuffs, 'paint');
    }
    ...
    if (immuneEv) pulseDebuff(refs.debuffs, 'immune');
    // Reactions own the slam: "OVERLOAD +7" beats the generic event text.
    if (reaction) {
      bang(slam, `${reaction.name} +${reaction.amount}${reaction.name === 'TRANSMUTE' ? 'g' : ''}`, 560);
      if (!fast && !accelerate) fxBurst(tick.element, .5, .5, Math.max(8, reaction.amount));
    } else if (immuneEv) {
      bang(slam, `${immuneEv.name} IMMUNE`, 150);
    } else {
    ...
    // A reaction gets a longer beat - the banner needs a breath to read.
    await wait(finale || reaction ? 260 : beat);
