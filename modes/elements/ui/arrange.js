...
export function initArrange(io) {
  ...
  function openSheet(d) {
    ...
    // Touch parity: the reaction line leads (hover tooltips already show it).
    $('sheetDesc').textContent = [speciesAbilityText(d), info.desc].filter(Boolean).join(' ');
  ...
  if (hoverable) {
    ...
    const kw = t => t
      .replace(/\b(OVERLOAD|SHATTER|IGNITE|CORRODE|TRANSMUTE)\b/g, '<b class="kw kw-react">$1</b>')
      ...
      .replace(/\bpaints?\b/gi, m => `<b class="kw kw-paint">${m}</b>`)
