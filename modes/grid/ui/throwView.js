...
export async function animateThrow(refs, r) {
  ...
  hits.innerHTML = '';
  boardShell(tray); // reuses the camp's unlocked-area dataset
  total.textContent = '0';
  ovk.textContent = '';
  ovk.className = 'ovk';
  ...
  for (const dr of r.diceRes) {
    ...
    positionGridSlot(slot, dr.cell, dr.size);
    slot.innerHTML = dieHtml(dr);
    tray.appendChild(slot);
    slots[dr.slot] = slot;
  }
  // The activation sweep: the lit square (.ignite) walks the READING ORDER
  // of the board - row by row, left to right - while the rest dims.
