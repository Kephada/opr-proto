...
import { BENCH_CAP, CLANS, LEVELS, MAX_HEARTS, RANK_AT, clanCounts, modifierLabel, rankFor } from '../engine/config.js';
import { PHASE, currentFoe } from '../engine/run.js';
import { showFoe } from './arena.js';
import { boardShell, positionGridSlot } from './board.js';
...
// The tray is the BOARD: 9 absolute cells (locked ones dark + chained), and
// one slot per fielded die spanning its footprint. The slot-num is the
// reading-order resolution number - the order the cascade will walk.
function renderActive(ctx) {
  const { run, selected } = ctx;
  const tray = $('tray');
  boardShell(tray, run.build.area);
  const sealed = currentFoe(run).sealedSlot === 0 && run.phase === PHASE.ARRANGE;
  run.build.active.forEach((d, i) => {
    const slot = document.createElement('div');
    slot.className = 'dice-slot';
    slot.dataset.zone = 'active';
    slot.dataset.i = i;
    positionGridSlot(slot, d.cell, d.size);
    if (selected?.zone === 'active' && selected.idx === i) slot.classList.add('sel');
    if (run.phase === PHASE.ARRANGE) slot.classList.add('movable');
    if (sealed && i === 0) slot.classList.add('locked'); // boss seals the first reading-order die
    slot.innerHTML = `<span class="slot-num">${i + 1}</span>${dieCardHTML(d)}`;
