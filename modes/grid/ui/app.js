...
import { dieCardHTML, footprintBadgeHTML } from './dieCard.js';
...
// THE GRID has its own key: the cell/area build format is not interchangeable
// with the base game's slot saves.
const SAVE_KEY = 'opr-grid-save-v1';

function loadSave() {
  try {
    const s = JSON.parse(localStorage.getItem(SAVE_KEY));
    if (!s || s.v !== 1 || !s.run?.build?.area) return null;
    if (s.run.build.active.some(d => !d.cell)) return null; // pre-grid die: not resumable
    return {
      ...s.run,
      phase: PHASE.ARRANGE,
      wager: false,
...
function altarMounts(offers) {
  return offers.map((o, i) => {
    ...
    return `<button class="altar-die" type="button" data-zone="altar" data-i="${i}" style="${pos}">${dieCardHTML(o.die)}${footprintBadgeHTML(o.die.size)}${badge}</button>`;
  }).join('');
}
...
function renderShopPanel() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const badge = o.combinesWith ? `<span class="combine-badge">COMBINE Ã¢â€ â€™ ${sizeLabel(o.combinesWith)}</span>` : '';
    rows.push(`<button class="shop-row${broke}" type="button" data-i="${i}" data-desc="${o.desc.replace(/"/g, '&quot;')}" style="--row:${i}">
      ...
      <span class="sr-name">${o.name}${footprintBadgeHTML(o.die.size)}${badge}</span>
