...
import { elementInfo, footprint } from '../engine/config.js';
import { speciesGlyph } from './dieGlyphs.js';

// Mini grid badge: the die's footprint as tiny cell squares, so draft and
// shop offers read space-wise at a glance (▯ = one board cell).
export function footprintBadgeHTML(size) {
  const fp = footprint(size);
  return `<span class="fp-badge" style="--fw:${fp.w};--fh:${fp.h}" aria-label="${fp.w} by ${fp.h} cells">${'<i></i>'.repeat(fp.w * fp.h)}</span>`;
}
