// PATH 3 "THE GRID" data. Pure game data and offer helpers; no DOM.
// v0.8 Clans rules on a 3x3 cell board: dice are SHAPES that occupy cells,
// resolution order = reading order of each die's top-left cell (row-major).

export const ACTIVE_CAP = 9; // informational: nine 1x1 cells = the swarm dream
...
// ---- The Grid: footprints, area milestones, placement ----------------------
// Shapes (w x h in cells): big dice are big slabs, swarms of d4s fill corners.
...
export const FOOTPRINT = Object.freeze({
  ...
  6: { w: 1, h: 2 },  // tall
  8: { w: 2, h: 1 },  // wide
  10: { w: 1, h: 3 }, // full column
  12: { w: 3, h: 1 }, // full row
});
export const footprint = size => FOOTPRINT[size] || FOOTPRINT[4];

// The board starts as the top-left 2x2; the old slot milestones unlock AREA:
// fight 3 opens the third column (3x2), fight 5 the third row (full 3x3).
export const START_AREA = Object.freeze({ w: 2, h: 2 });
export const AREA_MILESTONES = Object.freeze([
  { idx: 2, w: 3, h: 2 },
  { idx: 4, w: 3, h: 3 },
]);

export function areaForFight(levelIdx) {
  let area = START_AREA;
  for (const m of AREA_MILESTONES) if (levelIdx >= m.idx) area = m;
  return { w: area.w, h: area.h };
}

// Which fight number (1-based) unlocks a locked cell - for the chain labels.
export const cellUnlockFight = (x, y) => (y >= 2 ? 5 : x >= 2 ? 3 : 1);

export function dieCells(d, origin = d.cell) {
  const fp = footprint(d.size);
  const cells = [];
  for (let dy = 0; dy < fp.h; dy++) {
    for (let dx = 0; dx < fp.w; dx++) cells.push({ x: origin.x + dx, y: origin.y + dy });
  }
  return cells;
}

// A die fits at origin if its FULL footprint lies in unlocked, empty cells.
// `ignore` (id or array of ids) excludes dice mid-move; a die never blocks
// itself, so plain re-placement checks need no extra bookkeeping.
export function canPlaceAt(build, d, origin, ignore = null) {
  if (!d || !origin) return false;
  const fp = footprint(d.size);
  const area = build.area || START_AREA;
  if (origin.x < 0 || origin.y < 0 || origin.x + fp.w > area.w || origin.y + fp.h > area.h) return false;
  const skip = new Set(Array.isArray(ignore) ? ignore : ignore ? [ignore] : []);
  if (d.id) skip.add(d.id);
  const occupied = new Set();
  for (const o of build.active) {
    if (!o.cell || skip.has(o.id)) continue;
    for (const c of dieCells(o)) occupied.add(`${c.x},${c.y}`);
  }
  return dieCells(d, origin).every(c => !occupied.has(`${c.x},${c.y}`));
}

// First reading-order origin where the die fits, or null.
export function findFreeOrigin(build, d, ignore = null) {
  for (let y = 0; y < GRID_H; y++) {
    for (let x = 0; x < GRID_W; x++) {
      if (canPlaceAt(build, d, { x, y }, ignore)) return { x, y };
    }
  }
  return null;
}

// build.active stays SORTED in reading order of top-left cells at all times
// (every placement/removal re-sorts) so resolve.js keeps consuming a flat
// ordered array - "next die" needs no 2D adjacency.
export function sortActive(build) {
  build.active.sort((a, b) =>
    ((a.cell?.y ?? GRID_H) - (b.cell?.y ?? GRID_H)) || ((a.cell?.x ?? GRID_W) - (b.cell?.x ?? GRID_W)));
  return build;
}
...
export function die(element = CLAN.STONE, size = 4, extra = {}) {
  ...
  return {
    ...
    seed: !!extra.seed,
    cell: extra.cell ? { x: extra.cell.x, y: extra.cell.y } : null, // grid origin (top-left)
  };
}

export const cloneDie = d => ({ ...d, cell: d.cell ? { x: d.cell.x, y: d.cell.y } : null });

export function cloneBuild(build) {
  return {
    area: { ...(build.area || START_AREA) },
...
export function STARTER(rng = Math.random) {
  ...
  return {
    area: { ...START_AREA },
    active: [
      die(seedClan, 4, { seed: true, species, cell: { x: 0, y: 0 } }),
      die(seedClan, 4, { seed: true, species, cell: { x: 1, y: 0 } }),
    ],
    bench: [],
  };
}
...
// ---- Build helpers ----------------------------------------------------------
// New dice land on the first free board origin (reading order), else the
// bench. With BOTH full, the weakest fielded die makes way if the newcomer's
// shape fits the hole; as a last resort the newest bench die is replaced.
export function addDieToBuild(build, sourceDie) {
  const d = cloneDie(sourceDie);
  const origin = findFreeOrigin(build, d);
  if (origin) {
    d.cell = origin;
    build.active.push(d);
    sortActive(build);
  ...
  d.cell = null;
  if (build.bench.length < BENCH_CAP) {
    build.bench.push(d);
    return;
  }
  const w = weakestActiveIndex(build.active);
  const [out] = build.active.splice(w, 1);
  const o2 = findFreeOrigin(build, d);
  if (o2) {
    d.cell = o2;
    build.active.push(d);
    sortActive(build);
  } else {
    build.active.splice(w, 0, out);
    build.bench[BENCH_CAP - 1] = d;
  }
}
