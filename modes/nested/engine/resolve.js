...
import { CLAN, FOCUS_CHARGES, bulwarkBonus, clanRanks, cloneDie, dieTitle, jackpotWindow, speciesGoldPower, speciesPower } from './config.js';
...
function simulateTimeline(build, foe, rollState, opts) {
  ...
  // V2 COMBO: charge minted by bursts at STRIKE time. Always zero in preview
  // (preview rolls no bursts), so the previewed total stays a honest floor.
  const liveCharge = rolls.map(() => 0);
  ...
  // ---- NESTED burst triggers, read off the SHOWN faces (value + charge),
  // so a charge can steer a vessel into its jackpot socket. Deterministic:
  // the preview knows WHICH sockets fire; the inner rolls happen live at
  // strike time exactly like Ember explosions. A sealed vessel (boss) keeps
  // its passengers sealed too.
  // Evolution widens the jackpot: a ★ vessel's top socket fires on its top
  // (1 + evo) faces, capped at 3 - the nested answer to "evo means nothing".
  const burstAt = rolls.map((r, i) => {
    if (r.locked) return [false, false];
    const s = r.die.sockets || [];
    const jackpot = !!s[0] && shown[i] >= r.die.size - (jackpotWindow(r.die) - 1);
    return [jackpot, !!s[1] && shown[i] === 1];
  });
  ...
  // A fired socket rolls its die LIVE (raw face - inner dice contribute only
  // their rolled value, no species hooks, no floors), adds it as a bonus on
  // the vessel's tick, and chains if the inner roll hits one of the inner
  // die's own sockets. Depth cap 3: a die at depth 3 still bursts, but its
  // own passengers stay inside.
  const burstName = x => `${x.name} d${x.size}`;
  const burstOut = (tick, source, vessel, inner, socketIdx, depth, triggerFace) => {
    const value = rollRaw(inner.size, opts.rng);
    const dealt = addDamage(tick, value, source, { type: 'burst' });
    tick.events.push({
      type: 'burst-roll', socket: socketIdx, depth,
      vessel: `d${vessel.size}`, shown: triggerFace,
      name: burstName(inner), element: inner.element, size: inner.size,
      value, dealt,
    });
    // V1 COMBO: the passenger is a full die, not a raw number - its species
    // gift fires off the burst face. Self-contained hooks only (neighbor and
    // slot hooks have no meaning inside a vessel):
    //   Ember crits its top · Fang poisons on odd · Coin pays when the burst
    //   BEATS the face that freed it (the insurance-socket gold engine).
    if (inner.species === 'cinder' && value === inner.size) {
      const p = speciesPower(inner.size);
      addDamage(tick, p, source, { type: 'burst' });
      tick.events.push({ type: 'species', amount: p, label: `${inner.name} crits the burst` });
    }
    if (inner.species === 'fangling' && value % 2 === 1) {
      const p = speciesPower(inner.size);
      venomStacks.push({ source, amount: p });
      tick.events.push({ type: 'venom-apply', amount: p });
    }
    if (inner.species === 'gilded' && value > triggerFace) {
      const g = speciesGoldPower(inner.size);
      coinEvoGold += g;
      tick.events.push({ type: 'species-gold', amount: g, label: `${inner.name} bursts rich` });
    }
    // V2 COMBO: every burst CHARGES the next board die - live damage, and it
    // can open that die's jackpot when its turn comes (never cancels a
    // previewed promise). Chained bursts each feed forward: the escalation.
    if (rolls[source + 1] && !rolls[source + 1].locked) {
      liveCharge[source + 1] += value;
      tick.events.push({ type: 'burst-charge', amount: value, target: source + 1 });
    }
    if (depth >= 3 || !inner.sockets) return;
    if (inner.sockets[0] && value >= inner.size - (jackpotWindow(inner) - 1)) burstOut(tick, source, inner, inner.sockets[0], 0, depth + 1, value);
    if (inner.sockets[1] && value === 1) burstOut(tick, source, inner, inner.sockets[1], 1, depth + 1, value);
  };
  ...
  const resolveOne = (i, repeat = false) => {
    ...
    // mints a second one. Live burst charge (V2) rides the same lane.
    const charge = repeat ? 0 : boost[i] + liveCharge[i];
    ...
    // ---- NESTED bursts: triggers were fixed at the boost pass; the inner
    // rolls happen here, live, like explosions. Preview marks them pending.
    // Bursts fire once - a repeat strike (Tempest Echo) doesn't re-open the
    // sockets, mirroring how charge lands once.
    // Live burst charge can OPEN the jackpot at strike time ("charge counts
    // toward the top face", applied to bursts). It only ever ADDS a firing:
    // the previewed insurance promise is never cancelled, so green stays
    // a honest floor and the surprise is pure upside.
    const burstFire = [
      burstAt[i][0] || (!!(d.sockets || [])[0] && liveCharge[i] > 0
        && shown[i] + liveCharge[i] >= d.size - (jackpotWindow(d) - 1)),
      burstAt[i][1],
    ];
    if (!repeat && (burstFire[0] || burstFire[1])) {
      for (const s of [0, 1]) {
        if (!burstFire[s]) continue;
        if (opts.includeExplosions) burstOut(tick, i, d, d.sockets[s], s, 1, shown[i] + (s === 0 ? liveCharge[i] : 0));
        else tick.events.push({ type: 'pending-burst', socket: s, name: burstName(d.sockets[s]) });
      }
    }
  ...
  return {
    ...
    diceRes: rolls.map((r, i) => ({
      ...
      focusHint: '',
      sockets: r.die.sockets || [null, null],
      burstPending: burstAt[i],
    })),
    ticks,
    total,
    cleared,
