...
// PATH 4 NESTED: 5 main dice max - vessels carry the rest inside.
...
export function die(element = CLAN.STONE, size = 4, extra = {}) {
  ...
  return {
    ...
    seed: !!extra.seed,
    // NESTED: sockets[0] binds to the TOP face (jackpot), sockets[1] to
    // face 1 (insurance). Each holds a strictly smaller die or null.
    // Plain data only - saves are JSON.
    sockets: extra.sockets || [null, null],
  };
}

// Deep-clone sockets: a shallow spread would alias the inner dice and a
// combine/eject on the copy would corrupt the original (and the save).
export const cloneDie = d => ({
  ...d,
  sockets: (d.sockets || [null, null]).map(s => (s ? cloneDie(s) : null)),
});
...
export function dieDesc(d) {
  ...
  const evo = d.evo ? ` ★${d.evo}: its proc grows. Jackpot ⬡${socketBindLabel(d, 0)}.` : '';
...
// NESTED rule 4: socketed dice still count as FIELDED - stuffing two greens
// inside vessels keeps Poison rank while freeing board slots. Bench vessels
// are not fielded, so neither are their passengers.
function countClan(counts, d) {
  counts[d.element] = (counts[d.element] || 0) + 1;
  for (const s of d.sockets || []) if (s) countClan(counts, s);
}

export function clanCounts(build) {
  const counts = {};
  for (const d of build.active) countClan(counts, d);
  return counts;
}
...
// ---- Sockets (PATH 4 NESTED) ------------------------------------------------
// Socket 0 = top face (jackpot), socket 1 = face 1 (insurance). Top first.
export const SOCKET_BINDS = ['top face', 'face 1'];
export const socketBindFace = (vessel, idx) => (idx === 0 ? vessel.size : 1);

// Evolution widens the jackpot: ★1 fires on the top 2 faces, ★2+ on the top
// 3 (mirrors Ember's arming window) - big vessels stop being anti-burst and
// evo finally means something nested. Insurance stays face 1: a charge that
// rescues a 1 also waives the payout, which is the point of insurance.
export const jackpotWindow = d => 1 + Math.min(2, d.evo || 0);
export const socketBindLabel = (vessel, idx) => {
  if (idx !== 0) return '1';
  const w = jackpotWindow(vessel);
  return w > 1 ? `${vessel.size - w + 1}+` : `${vessel.size}`;
};

export function freeSocketIndex(vessel) {
  const s = vessel?.sockets || [];
  return !s[0] ? 0 : !s[1] ? 1 : -1;
}

// Only a STRICTLY SMALLER die fits inside a vessel (d4 into d6+, d6 into
// d8+...). The inner die may carry its own passengers - that is the chain.
export function canSocket(inner, vessel) {
  return !!inner && !!vessel && inner !== vessel
    && inner.size < vessel.size && freeSocketIndex(vessel) >= 0;
}

// Every die the player HOLDS, passengers included. Socketing must never hide
// a die from pity/pairing - it is still a held copy of that identity (eject
// at camp to pair it).
export function allHeldDice(build) {
  const out = [];
  const walk = d => { out.push(d); for (const s of d.sockets || []) if (s) walk(s); };
  for (const d of [...build.active, ...build.bench]) walk(d);
  return out;
}

// All passengers across two vessels, smallest first - the eject order when a
// fuse overflows. null = no overflow (2 or fewer ride along fine).
export function socketOverflow(a, b) {
  const pool = [a.sockets?.[0], a.sockets?.[1], b.sockets?.[0], b.sockets?.[1]].filter(Boolean);
  return pool.length > 2 ? pool.sort((x, y) => x.size - y.size) : null;
}

// Merge two vessels' passengers into one pair of sockets, preferring to keep
// each die on its original binding. null = they don't fit (3+ passengers).
function mergedSockets(a, b) {
  const pool = [a.sockets?.[0], b.sockets?.[0], a.sockets?.[1], b.sockets?.[1]].filter(Boolean);
...
// NESTED: passengers ride along; if the two vessels hold 3+ dice between
// them the fuse is blocked (a combine must never delete a socketed die).
export const combineIdentity = (a, b) => !!a && !!b && a !== b
  && a.element === b.element && a.species === b.species
  && a.size === b.size && a.size < 12;

export function canCombine(a, b) {
  return combineIdentity(a, b) && mergedSockets(a, b) !== null;
}
...
export function combinedDie(a, b) {
  return die(a.element, nextTier(a.size), {
    ...
    species: a.species,
    sockets: (mergedSockets(a, b) || [null, null]).map(s => (s ? cloneDie(s) : null)),
  });
}
...
export const SLOT_MILESTONES = Object.freeze({ 4: 2, 5: 4 });
...
// NESTED retune: the base ladder was validated at a 6-die endgame; this mode
// caps the board at 5 (fights 1-6 have identical capacity, so only the last
// four targets shed roughly one die's expected contribution: 27->24, 31->27,
// 36->31, 46->40). Greedy-bot check (botcheck.mjs is the instrument; no
// wager, no arranging, board-first socketing): 2026-06-13 BURST COMBO pass
// (V1 passenger gifts + V2 burst-charges-next), 2x1000 runs: bare 227/1000
// (22.7%), socketing 283/1000 (28.3%) - the combo gives sockets a real
// +5.6pp edge (pre-combo the two sat at parity ~25%). Earlier history: the
// every-second-door cadence cratered both to ~1-2% (board starved at 2.4
// dice avg); drafts are back to every kill so nesting, not income, is the
// exam variable. Humans play well above this floor.
export const LEVELS = [
  ...
  { name: 'The Other One', target: 24, art: 'orc', desc: 'You killed its brother. It counted.' },
  { name: 'The Bigger One', target: 27, art: 'ogre', desc: 'You heard it three fights ago. It heard you first.' },
  { name: 'The Debt Collector', target: 31, art: 'lantern', desc: 'It holds a lantern for everyone you owe.' },
  { name: 'The Doorkeeper', target: 40, sealedSlot: 0, art: 'boss', desc: 'It knows your opening move. It has sealed it.' },
];
...
// NESTED: vessels need a lifelong supply of passengers. The main curve
// starves d4s late (5% by fights 7+), so the THIRD draft slot rolls small
// from fight 4 on - smalls stay drafted forever, sockets stay fed.
const PASSENGER_ODDS = [0.60, 0.40, 0, 0, 0];
...
export function unpairedIdentities(build) {
  const tally = {};
  for (const d of allHeldDice(build)) {
...
export function holdsIdentical(build, d) {
  return allHeldDice(build).some(x =>
...
export function draftOffers(levelIdx, build = { active: [], bench: [] }, rng = Math.random, forceDup = false) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    // Slot 3 is the passenger lane from fight 4 on (pity may still claim it).
    const slotOdds = (i === 2 && levelIdx >= 3) ? PASSENGER_ODDS : odds;
    if (pity.length && rng() < pityChance) {
      ...
      d = die(clan, rollTier(slotOdds, rng), { species: pickSpecies(clan, rng) });
    }
    offers.push(d);
  }
  ...
  // The FIRST draft teaches the socket dilemma: guarantee a d4 of the seed
  // species - it pairs with the seed dice now, or rides inside the first
  // combine later. Same die, two futures; the mode's core choice in
  // miniature, engineered, not scripted.
  if (levelIdx === 0) {
    const seed = [...build.active, ...build.bench].find(d => d.seed);
    if (seed && !offers.some(o => o.size === 4 && o.element === seed.element)) {
      offers[0] = die(seed.element, 4, { species: seed.species });
    }
  }
  return offers.map(d => ({
    type: 'recruit',
...
// Shop: dice only, priced by tier (+20% for Coinblood), pity applies too.
