...
import {
  ...
  SHOP_AFTER, SHOP_REROLL_COST, STARTER, activeSlots, canCombine, canSocket,
  clanRanks, cloneBuild, combineIdentity, combinedDie, difficultyInfo, draftOffers,
  freeSocketIndex, killGold, overkillDivisor, pickTheme, shopOffers,
  slotsForFight, socketOverflow, themedLevels,
} from './config.js';
...
export function strikeCurrentFight(run, rng = Math.random) {
  ...
  if (won) {
    // Cadence: a draft after EVERY kill. The 2026-06-11 every-second-door
    // experiment starved the 5-slot board (bot held 2.4 dice avg, deaths
    // clustered fights 3-6); the kill/keep exam must isolate NESTING as
    // the variable, so the income knob stays at the v0.8 default.
...
// NESTED: when both vessels carry passengers (3+ total), the smallest
// passengers step out onto the bench first - a fuse must never delete a
// die, but a full bench can still block it.
export function combineDice(run, zoneA, idxA, zoneB, idxB) {
  ...
  const b = listB[idxB];
  if (!canCombine(a, b)) {
    if (!combineIdentity(a, b)) return run;
    const overflow = socketOverflow(a, b);
    if (!overflow) return run;
    // Eject smallest-first until only 2 passengers remain or the bench fills.
    let pool = overflow;
    while (pool.length > 2 && build.bench.length < BENCH_CAP) {
      const out = pool[0];
      for (const v of [a, b]) {
        const k = (v.sockets || []).indexOf(out);
        if (k >= 0) { v.sockets[k] = null; break; }
      }
      build.bench.push(out);
      pool = pool.slice(1);
    }
...
// SOCKET (PATH 4 NESTED): a strictly smaller die dropped on a bigger die
// climbs inside - top-face socket first, then the face-1 insurance socket.
// The inner die leaves the board/bench. Allowed wherever combine is.
// socketIdx lets the die-sheet inventory aim a SPECIFIC empty socket
// (jackpot vs insurance is a real build choice); -1 keeps top-first.
export function socketDie(run, srcZone, srcIdx, dstZone, dstIdx, socketIdx = -1) {
  if (run.phase !== PHASE.ARRANGE && run.phase !== PHASE.REWARD && run.phase !== PHASE.SHOP) return run;
  if (srcZone === dstZone && srcIdx === dstIdx) return run;
  const build = cloneBuild(run.build);
  const srcList = srcZone === 'bench' ? build.bench : build.active;
  const dstList = dstZone === 'bench' ? build.bench : build.active;
  const inner = srcList[srcIdx];
  const vessel = dstList[dstIdx];
  if (!canSocket(inner, vessel)) return run;
  const k = socketIdx >= 0 && !vessel.sockets[socketIdx] ? socketIdx : freeSocketIndex(vessel);
  vessel.sockets[k] = inner;
  srcList.splice(srcIdx, 1); // object refs survive the splice
  return { ...run, build };
}

// EJECT (camp only): the passenger steps out onto the bench. The UI disables
// the button when the bench is full; the guard here makes it a no-op anyway.
export function ejectSocket(run, zone, idx, socketIdx) {
  if (run.phase !== PHASE.ARRANGE) return run;
  const build = cloneBuild(run.build);
  const vessel = (zone === 'bench' ? build.bench : build.active)[idx];
  const inner = vessel?.sockets?.[socketIdx];
  if (!inner || build.bench.length >= BENCH_CAP) return run;
  vessel.sockets[socketIdx] = null;
  build.bench.push(inner);
  return { ...run, build };
}
