...
import { elementInfo, socketBindLabel } from '../engine/config.js';
...
// NESTED: tiny socket pips under the sigil - the inner die's sigil plus the
// bound face ("⬡6" jackpot / "⬡1" insurance). `pending` lights a pip whose
// burst the preview has already promised.
function socketPipsHTML(d, pending = []) {
  const s = d.sockets || [];
  const pips = [0, 1].map(k => {
    const inner = s[k];
    if (!inner) return '';
    const lit = pending[k] ? ' lit' : '';
    return `<span class="socket-pip el-${inner.element}${lit}">${speciesGlyph(inner.species) || '<i class="sp-dot"></i>'}<b>⬡${socketBindLabel(d, k)}</b></span>`;
  }).join('');
  return pips ? `<span class="socket-pips">${pips}</span>` : '';
}

// face: numeric value -> pips (+ tiny numeral badge); 'LOCK' -> seal; '?' ->
// pips are flickered live by rollView; unrolled -> element gem + size tag.
export function dieCardHTML(d, { label = null, armed = false, tumbling = false, pending = [] } = {}) {
  ...
  return `${orb}<div class="${cls}"${title}><span class="v">${face}</span>${badge}</div>${sizeTag}${socketPipsHTML(d, pending)}`;
}

// Adapter for diceRes rows (strike/roll replays). Sockets and pending-burst
// flags ride along so the replay cards keep their pips lit.
export function dieCardFromRes(dr, opts = {}) {
  return dieCardHTML(
    { element: dr.element, orb: dr.orb, name: dr.short, size: dr.size, evo: dr.evo, sockets: dr.sockets },
    { pending: dr.burstPending || [], ...opts },
  );
}
