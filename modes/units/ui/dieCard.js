...
import { elementInfo, itemInfo } from '../engine/config.js';
...
export function dieCardHTML(d, { label = null, armed = false, tumbling = false } = {}) {
  ...
  } else if (label === 'LOCK' || label === 'KO') {
    face = `<span class="face-text">${label}</span>`;
  } else {
...
// ---- UNITS: the creature chrome above every fielded die --------------------
// Tiny item pip (one equipment slot): a lettered tag in the frame corner.
export function itemPipHTML(itemId) {
  const it = itemInfo(itemId);
  return it ? `<span class="item-pip ip-${it.id}" aria-label="${it.name}">${it.letter}</span>` : '';
}

// hp pips: one per maxHp, lit while alive. Re-rendered live by throwView
// when the foe's counter lands.
export function hpPipsHTML(hp, maxHp) {
  let pips = '';
  for (let k = 0; k < maxHp; k++) pips += `<i class="${k < hp ? 'on' : ''}"></i>`;
  return `<span class="hp-pips${hp <= 0 ? ' dead' : ''}">${pips}</span>`;
}

// The portrait: species sigil large in a circular frame, hp pips under it,
// item pip at its shoulder. u = {slot,name,species,element,size,item,hp,maxHp,ko}.
export function unitFrameHTML(u) {
  if (!u) return '';
  return `<span class="unit-frame el-${u.element}${u.ko ? ' ko' : ''}">
    <span class="unit-portrait">${speciesGlyph(u.species) || '<span class="up-gem"></span>'}</span>
    ${hpPipsHTML(u.hp, u.maxHp)}
    ${u.item ? itemPipHTML(u.item) : ''}
  </span>`;
}
