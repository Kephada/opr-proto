...
import { dieCardFromRes, faceNumHTML, unitFrameHTML } from './dieCard.js';
...
function tumblingDieHtml(dr, unit) {
  const label = dr.locked ? (dr.ko ? 'KO' : 'LOCK') : '?';
  return `<span class="slot-num">${dr.slot + 1}</span>${unitFrameHTML(unit)}${dieCardFromRes(dr, { label, tumbling: !dr.locked })}`;
}

function settle(slot, dr, idx, unit) {
  slot.innerHTML = `<span class="slot-num">${dr.slot + 1}</span>${unitFrameHTML(unit)}${dieCardFromRes(dr, { label: dr.label, armed: dr.armed })}`;
  slot.querySelector('.die').classList.add('show');
  const ring = document.createElement('span');
  ring.className = 'impact';
  slot.appendChild(ring);
  if (dr.ko) return; // a downed unit makes no sound - it just lies there
  if (dr.locked) {
...
export async function animateRoll(refs, preview) {
  ...
  const slots = preview.diceRes.map(dr => {
    ...
    slot.innerHTML = tumblingDieHtml(dr, units[dr.slot]);
  ...
  for (let i = 0; i < slots.length; i++) {
    ...
    settle(slots[i], preview.diceRes[i], i, units[preview.diceRes[i].slot]);
  }
