...
import { ACTIVE_CAP, BENCH_CAP, CLANS, LEVELS, MAX_HEARTS, RANK_AT, SLOT_MILESTONES, activeSlots, clanCounts, modifierLabel, rankFor, unitMaxHp } from '../engine/config.js';
import { PHASE, currentFoe, foeAttackPlan, unitHp, unitKO } from '../engine/run.js';
import { showFoe } from './arena.js';
import { debuffIcon } from './debuffIcons.js';
import { dieCardHTML, itemPipHTML, unitFrameHTML } from './dieCard.js';
...
function renderActive(ctx) {
  ...
  for (let i = 0; i < ACTIVE_CAP; i++) {
    ...
    // UNITS: the die is a creature - portrait, hp pips, item pip, KO grey.
    const u = {
      slot: i, name: d.name, species: d.species, element: d.element,
      size: d.size, item: d.item || null,
      hp: unitHp(run, d), maxHp: unitMaxHp(d), ko: unitKO(run, d),
    };
    if (u.ko) slot.classList.add('ko');
    const label = dr ? dr.label : null;
    slot.innerHTML = `<span class="slot-num">${i + 1}</span>${unitFrameHTML(u)}${dieCardHTML(d, { label, armed: !!dr?.armed })}`;
...
function renderBench(ctx) {
  ...
  for (let i = 0; i < BENCH_CAP; i++) {
    ...
    slot.dataset.i = i;
    slot.innerHTML = `<span class="slot-num">B${i + 1}</span>${d ? dieCardHTML(d) + itemPipHTML(d.item) : ''}`;
...
function renderStatus(ctx) {
  ...
  const c = run.lastStrike?.counter;
  const recap = run.lastStrike && !run.lastStrike.cleared
    ? (c?.wipe
      ? `<span class="bad">team wiped — ♥ −1 · the squad regroups</span> · roll again`
      : `<span class="bad">short by ${run.lastStrike.short} — it struck your line</span> · roll again`)
    : '';
...
function renderFoe(ctx) {
  ...
  const foe = currentFoe(run);
  // UNITS: the foe's counter-attack plan is table stakes - it decides the
  // marching order. Short form on the always-visible plate, full sentence
  // on the enemy card.
  const plan = foeAttackPlan(run);
  const planShort = plan.pattern === 'aoe' && !plan.alternates ? `ATK ${plan.aoe} · all`
    : plan.alternates ? `ATK ${plan.attack} · 2↔all`
      : plan.pattern === 'dual' ? `ATK ${plan.attack} · front 2`
        : `ATK ${plan.attack} · front`;
  $('roomname').textContent = foe.name;
  const levels = run.levels || LEVELS;
  $('roomprop').textContent = `${Math.min(run.levelIdx + 1, levels.length)}/${levels.length} · ${planShort}`;
  ...
  $('ecMod').textContent = foe.sealedSlot === 0 ? 'Seals slot 1' : (modifierLabel(foe.modifier) || 'No modifier');
  $('ecAttack').textContent = `Attacks: ${plan.label}`;
  ...
  // Status icons (WoW target-frame): armor/ward it WEARS, poison it CARRIES.
  const statuses = [];
