...
import { dieCardFromRes, hpPipsHTML, pipsHTML, unitFrameHTML } from './dieCard.js';
...
function eventText(events) {
  ...
  const boomSum = events.filter(e => e.type === 'explosion').reduce((s, e) => s + e.amount, 0);
  ...
  if (events.some(e => e.type === 'ko')) return ''; // a downed unit gets no slam
...
function dieHtml(dr, unit) {
  return `<span class="slot-num">${dr.slot + 1}</span>${unitFrameHTML(unit)}${dieCardFromRes(dr, { label: dr.label, armed: dr.armed })}`;
}

function logLine(refs, text, cls = '') {
  if (!refs.log) return;
  refs.log.insertAdjacentHTML('beforeend', `<div class="log-line ${cls}">${text}</div>`);
  refs.log.scrollTop = refs.log.scrollHeight;
}

// One aimed hit of the foe's counter: the unit's slot flashes red, a -N
// floater pops off it, its hp pips drain, KO greys the whole slot.
function hitUnit(refs, slot, hit) {
  if (!slot) return;
  slot.classList.remove('unit-hit');
  void slot.offsetWidth;
  slot.classList.add('unit-hit');
  tone(68, .09, 'sine', .07); // thud
  tone(150, .04, 'square', .03);
  if (!fast && !accelerate) {
    const rct = slot.getBoundingClientRect();
    const f = document.createElement('span');
    f.className = 'unit-float';
    f.textContent = `−${hit.amount}`;
    f.style.left = `${rct.left + rct.width / 2}px`;
    f.style.top = `${rct.top - 4}px`;
    document.body.appendChild(f);
    f.addEventListener('animationend', () => f.remove());
  }
  const pips = slot.querySelector('.hp-pips');
  if (pips) pips.outerHTML = hpPipsHTML(hit.hpAfter, hit.maxHp);
  if (hit.ko) {
    slot.classList.add('ko');
    slot.querySelector('.unit-frame')?.classList.add('ko');
  }
}

// The foe's counter-attack replay: aimed by its pattern into the marching
// order. AOE rakes every slot fast; front/dual land as deliberate blows.
async function playCounter(refs, slots, c) {
  await wait(420); // the lunge lands first
  const aim = c.pattern === 'aoe' ? `ALL units for ${c.per}`
    : c.pattern === 'dual' ? `the front two for ${c.per} each`
      : `the front unit for ${c.per}`;
  logLine(refs, `it strikes back — ${aim}`, 'lose');
  for (const hit of c.hits) {
    hitUnit(refs, slots[hit.slot], hit);
    logLine(refs, `foe claws ${hit.name} for ${hit.amount}`, `el-${hit.element}`);
    if (hit.thorns) {
      spawnFloat(refs, hit.thorns, 'venom', ' small');
      logLine(refs, `${hit.name}'s thorns bite back for ${hit.thorns}`, 'el-venom');
    }
    if (hit.ko) logLine(refs, `<b>${hit.name} is KNOCKED OUT</b>`, 'lose');
    await wait(c.pattern === 'aoe' ? 90 : 220);
  }
  if (c.wipe) {
    playerHit(refs);
    refs.result.textContent = 'TEAM WIPE · ♥ −1';
    refs.result.className = 'result show lose';
    const sheet = document.querySelector('.char-panel');
    if (sheet) {
    ...
    logLine(refs, c.revived
      ? 'TEAM WIPE — ♥ −1 · the squad staggers back up'
      : 'TEAM WIPE — ♥ −1', 'verdict lose');
    await wait(360);
  }
}
...
export async function animateThrow(refs, r) {
  ...
  for (const dr of r.diceRes) {
    ...
    slot.className = `dice-slot rolled${dr.locked ? ' locked' : ''}${dr.ko ? ' ko' : ''}`;
    slot.innerHTML = dieHtml(dr, r.units?.[dr.slot]);
  ...
  if (refs.log) {
    refs.log.insertAdjacentHTML('beforeend', r.cleared
      ...
      : `<div class="log-line verdict lose">it stands - ${r.short} left</div>`);
    refs.log.scrollTop = refs.log.scrollHeight;
  }
  if (r.cleared) {
    ...
    // then the foe LUNGES at the camera and strikes back - but UNITS take
    // the blow now, aimed by the pattern. Hearts only bleed on a team wipe
    // (playCounter handles that escalation).
    ...
    result.textContent = `SHORT BY ${r.short}`;
    result.className = 'result show lose';
    ovk.textContent = '';
    sting();
    if (!fast && !accelerate) boom();
    if (r.counter) await playCounter(refs, slots, r.counter);
  }
}
